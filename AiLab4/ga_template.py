...
def reproduce(mother, father):
    '''
    Reproduce individuals with single-point crossover
    Return the child individual
    '''
    sex = random.randint(0,len(mother))
    child = mother[:sex] + father[sex:]
    return child
...
def random_selection(population, fitness_fn):
    ...
    for individual in ordered_population:
        totalFitness += fitness_fn(individual)
        fitnessList.append(fitness_fn(individual))
    selectionList = []
    for individual in ordered_population:
        fitness = fitness_fn(individual)
        for i in range(0,fitness):
            selectionList.append(individual)
    father = random.choice(selectionList)
    mother = random.choice(selectionList)
...
def fitness_function(individual):
    ...
    '''
    return individual[0]*4+individual[1]*2+individual[2]*1
