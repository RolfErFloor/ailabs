...
class Variable(object):
    ...
    def calculate_marginal_probability(self):
        if self.ready:
            return
        if not self.parents:
            for assignment, idx in self.assignments.items():
                self.marginal_probabilities[idx] = self.probability_table[()][idx]
        else:
            for row_key, prob_dist in self.probability_table.items():
                parent_prob= 1.0
                for i, parent in enumerate(self.parents):
                    parent_prob *= parent.get_marginal_probability(row_key[i])
                for assignment, idx in self.assignments.items():
                    self.marginal_probabilities[idx] += prob_dist[idx] * parent_prob
...
class BayesianNetwork(object):
    ...
    def get_joint_probability(self, values):
        ...
        joint_prob = 1.0
        for var in self.variables:
            var_val = values[var.name]
            parent_vals = self.sub_vals(var, values)
            joint_prob *= var.get_probability(var_val, parent_vals)
